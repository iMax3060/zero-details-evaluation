...
//void worker_thread(FreeList& freelist, std::array<uint_fast32_t, block_count>& pageIDs, std::array<std::atomic_flag, block_count>& pageUnused) {
//    if (freelist.useCDSThreadManagement()) cds::threading::Manager::attachThread();
//    for (unsigned int i = 0; i < iteration_count; i++) {
//        freelist.use(pageIDs, pageUnused);
//    }
//    if (freelist.useCDSThreadManagement()) cds::threading::Manager::detachThread();
//}
...
//            worker_threads[i] = new std::thread([&]{worker_thread<freelist.use>(pageIDs, pageUnused);});
